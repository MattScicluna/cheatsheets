...
%of a \textbf{Vector Space} $V$ over a subfield $F$ as a mapping $V \mapsto \mathbb{R}$ satisfying the following properties $\forall a\in F, u,v\in V$:
%\begin{itemize}
%	\item $||av||=|a|||v||$ \emph{Absolutely Scalable}
%	\item $||u+v||\le ||u||+||v||$ \emph{Triange Inequality}
%	\item $||u||\ge0$ \emph{Non Negative}
%	\item $||u||=0 \Rightarrow u=0$ \emph{Point-Seperating}
%\end{itemize}
...
\begin{theorem}\label{eq:convex1st}
...
\begin{theorem}[Slater's Condition]
	Suppose we have a convex primal, and that int($\mathcal{D}$) is non empty. If $\exists x \in$int($\mathcal{D}$)\footnote{The conditions that int($\mathcal{D}$) is non empty and $\exists x \in$int($\mathcal{D}$) can be generalized to $\exists x \in$relint($\mathcal{D}$)} satisfying $f_1(x)<0, \cdots, f_m(x)<0$ and $h_1(x)=0, \cdots, h_p(x)=0$, we have strong duality. If any $f_i$ are affine, we only need them to satisfy $f_i(x)=0$.
\end{theorem}
\begin{proof}
...
We first describe a property of functions that will be needed. We say a function $f$ is $L$ \textbf{Lipschitz Continuous} if 
\begin{align}
|f(x)-f(y)|=L||x-y||^2
\end{align}
Such a function has the following property: $||\nabla_xf(x)||\le L$. The Gradient Descent algorithm iterates in the following way, given $x_t$ set 
\begin{align}\label{eq:gdeq}
x_{t+1}=x_t-\gamma\nabla_x f(x_{t})
\end{align}
and iterate until convergence (or boredom). The motivation for this is to minimize a \emph{linear approximation} of $f$ -- the first order Taylor Expansion (contrast this with Newtons method). We call $\gamma$ the \textbf{Learning Rate}\footnote{This is also called the \textbf{Step Size} in Optimization}. The following theorem provides a way to choose the step size.

\begin{theorem}
	Let $f$ be convex and $L$-Lipschitz then if we choose $\gamma=\frac{|| x_1 - x^*||}{L\sqrt{T}}$, we have that $$f\left( \frac{1}{T}\sum_{i=1}^T x_t \right) - f(x^*) \le \frac{||x_1 - x^*||L}{\sqrt{T}}$$
\end{theorem}
\begin{proof}
	We use the convexity of $f$ and (\ref{eq:convex1st}) to get that:
	\begin{align*}
	f(x_t) - f(x^*) &\le \nabla_x f(x_t)^T(x_t-x^*)\\
	&\overset{(a)}{=}-\frac{1}{\gamma}(x_{t+1}-x_t)^T(x_t-x^*)\\
	&= \frac{1}{2\gamma}\Bigl( ||x_t-x^*||^2 - ||x_{t+1}-x^*||^2 + \gamma^2 || \nabla_x f(x_t) ||^2\Bigr)\\
	&\overset{(b)}{=} \frac{1}{2\gamma}\Bigl( ||x_t-x^*||^2 - ||x_{t+1}-x^*||^2 \Bigr) + \frac{\gamma}{2}L^2\\
	\end{align*}
	where (a) follows by (\ref{eq:gdeq}) and (b) follows from the Lipschitz continuity of $f$. We denote
\end{proof}
...
We do not know the \text{Generalization Error} since we do not know $p$, but we can approximate it using the \textbf{Empirical Risk Minimization} (ERM):
