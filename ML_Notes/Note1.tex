...
We now discuss how to solve optimization problems. This section is largly based off of \cite{Boyd:2004:CO:993483}. We want to minimize our \textbf{Objective Function} $f_0 : \mathcal{D} \rightarrow \mathbb{R}$ w.r.t some our \textbf{Optimization Variable} $x \in \mathbb{R}^n$ subject to some \textbf{Inequality Constraints} $f_i: \mathbb{R}^n \rightarrow \mathbb{R}$ and some \textbf{Equality Constraints} $h_i: \mathbb{R}^n \rightarrow \mathbb{R}$. Formally:
\begin{align*}
\text{Minimize } & f_0(x) \\
...
We call a point $x$ \textbf{Feasible} if $x\in \mathcal{D}$ and satisfies the constraints. We call $x$ \textbf{Strictly Feasible} if $x\in$ int($\mathcal{D}$) and satisfies $f_1(x)<0, \cdots, f_m(x)<0$ and $h_1(x)=0, \cdots, h_p(x)=0$.
The \textbf{Optimal Value} of this problem is $p^*=\inf\{f_0(x): \text{x satisfies constraints}\}$. We let $p* = \infty$ if there are no feasible points, and $p^*=-\infty$ is the problem is unbounded from below. If $x$ feasible and $f_0(x) = p^*$ then we call it \textbf{Optimal}.
...
Instead of minimizing $f_0$ to get $p^*$ we can maximize the lower bound $g$. This is an easier problem since $g$ is concave. It is clear that we always have \textbf{Weak Duality} $p^* \ge d^*$, although we want \textbf{Strong Duality}: $p^* = d^*$. A sufficient condition for strong duality is \textbf{Slater's Condition}

\begin{theorem}[Slater's Condition]
	Suppose we have a convex primal (i.e. $f_0, f_i$ convex and $h_i$ affine), if there exists a strictly feasible $x$ we then have strong duality
\end{theorem}
...
This provides a nice interpretation for the MLE - it is finding the $p\in \{p_{\theta}\}_{\theta\in\Theta}$ which minimizes the dissimilarity between the empirical distribution of the training set and itself as measured by the KL divergence. Conversly we can justify the use of the Cross Entropy loss through its equivalence to Maximum Likelihood.
...
Let $X$ be finite with $\mathcal{X}$ and $k$ as defined before. Suppose we have feature functions $T_1(X), \cdots, T_d(X)$ and we define $\mathcal{M}$ as
\begin{equation}
...
Where our testible information are $d$ \emph{moment constraints}. Using the relation \(H(p)=\log k-D(p||q)\) derived from theorem \ref{th:unifentlim} we have the following alternative characterization of MaxENT:
\begin{align}
