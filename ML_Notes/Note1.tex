...
We call \(\delta\) a \textbf{Learning Algorithm} which learns $f$ using the training data i.e. $\delta(D) = f$. As before, we can evaluate the performance of $f$ using $V$. In this context $V$ is a measure of the distance between a given \textbf{Prediction} $f(x_i)$ and it’s associated \textbf{Ground Truth} $y_i$.
...
\subsubsection{Validation Sets and Test Sets}
